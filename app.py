...
app.layout = dbc.Container(fluid=True, children=[
    ...
    dbc.Row([
        dbc.Col(dash_table.DataTable(id='top-vendedores', style_table={'overflowX': 'auto'}), md=12)
    ]),
...
def update_dashboard(contents):
    ...
    kpis = f"Total: {total_gerados} | Utilizados: {total_utilizados} | Conversão: {conversao:.2f}% | Ticket Médio: R$ {ticket_medio:,.2f}"
    ...
    top_vendedores_df = df_utilizados.groupby(
        ['Nome do vendedor', 'Nome da filial', 'Nome da rede']
    ).size().reset_index(name='Quantidade').sort_values(by='Quantidade', ascending=False).reset_index(drop=True)
    top_vendedores_df.index += 1
    top_vendedores_df.insert(0, 'Ranking', top_vendedores_df.index)

    vendedores_columns = [{"name": i, "id": i} for i in top_vendedores_df.columns]

    fig_dispositivos = px.bar(df['Descrição'].value_counts().nlargest(10).reset_index(),
                              x='index', y='Descrição', title='Top 10 Dispositivos Mais Avaliados')

    return kpis, fig_gerados, fig_utilizados, fig_ticket, top_vendedores_df.to_dict('records'), vendedores_columns, fig_dispositivos
